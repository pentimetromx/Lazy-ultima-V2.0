...
let arrayIdButtsCheck = ['boton2','boton3','boton4','boton5','boton6','boton7','boton8','boton9','boton10','boton11','boton12','btn600','btn700','btn800']
let idsMAhijos = ['contBotCasos','materiales-kaizen','troubleshooting','canvasContainer2','canvasContainer3','contImagNeg','contImagGraf']
...
let buttsToyota = ['toyota8', 'toyota7', 'toyota6', 'toyota5','toyota4','toyota3','toyota2','toyota1']
let allContIzquierdos = ['desbobinadorId','uTeñidos','alimentadorId','unidProceso','rebobinador']
let arrayIdButtsMA = ['resultados','troubleshoot','def1','def2','adtBut','fua1','lup','kaizen']
const inputNuevoEmpl = document.querySelectorAll('.form-empleado input, .form-empleado select');
let arrayButtsRutilantesIII = ['butt-001130','butt-0012','butt-0016','butt-0021','butt-00202']
...
let contiBtt = ['archivo','btt2','btt3','btt4','btt5','btt6','btt7','btt8'] 
...
let contiVidEntintador = document.getElementById('conti-video-entintador') 
let contImgsDistribuidor = document.getElementById("images-distribuidor")
...
let buttRepuestV = document.getElementById('conti-boton-repuestos-III') 
...
let positionDisplayIII = document.getElementById('positionDisplay_3')
let miCanvas = document.getElementById('MiGrafica').getContext('2d')
let interfazPerfiles = document.getElementById('perfiles-entintado')
let contComercial = document.getElementById('cont-titulo-comercial')
let contOperativa = document.getElementById('cont-titulo-operacion')
let contibotsDistriIII = document.getElementById('contenedor-7-VI')
let contVideoSmed = document.getElementById('conti-video-bancada')
let contibotsDistriV = document.getElementById('contenedor-7-VII')
let buttonsMA = document.getElementsByClassName("butt-mautonomo")
let posicionPantalla = document.getElementById('positionDisplay')  
...
let arrayImgsRodilleria = document.querySelectorAll ('.durezas')
const panelDeBajas = document.querySelector('#panel-principal')
const padreDos = document.getElementById('contenedor_padre_2')
const padreTres = document.getElementById('contenedor_padre_3') 
let cortinaBloqueo = document.querySelector('#cortina-bloqueo')
const botonesFicha = document.querySelectorAll('.fichas-butts')
let contibotsDistriVI = document.getElementById('contenedor-8')
let contiVidSmed = document.getElementById('conti-video-smed')
...
let contibotsDistri = document.getElementById('contenedor-7')
let enlacesIDs = ['link1', 'link2', 'link3', 'link4','link5'] 
const padreUno = document.getElementById('contenedor_padre') 
let contPadre = document.getElementById('conti-boton-freno')
let btnsIniciales = document.querySelectorAll('.btn-bloque')
let linksIniciales = ['links-inicialesI','links-iniciales']
let buttRepuest = document.getElementById('butt-repuestos')
...
const cortina = document.getElementById('overlay-cortina');
const granCortina = document.getElementById('overlay-general');
...
const interfazColor = document.querySelector('#interfaz-perfiles')
const interfazRRHH = document.querySelector('#padre-ingresos')
const interfazMA = document.querySelector('#padre-ingresos-ma')
const interfazVisor = document.querySelector('#padre-porta-visor')
...
const mejoraFichaTec = document.querySelector ('#mejora')
const equiposMA = document.querySelector('#equipo')
const campoBusqueda = document.getElementById('buscador-empleado');
const botoneraKaizen = document.querySelector('#conti-boton-kaizen')
const elementoPadre = document.getElementById('cont-titulo');
const listaFotos = document.getElementById('listaFotos');
const bloqueador = document.querySelector('.bloqueador')
...
const empleadosEnLocalStore = JSON.parse(localStorage.getItem('empleadosRegistrados')) || [];
const kaizenEnLocalStore = JSON.parse(localStorage.getItem('kaizenRegistrados')) || [];
const coloresEnLocalStore = JSON.parse(localStorage.getItem('coloresRegistrados')) || [];
...
function mostrarPorClaveLocalStore(clave) {
  ...
  const data = Array.isArray(raw)
    ? raw
    : Object.values(raw);
  console.group(`Contenido de ${clave}`);
  console.table(data);
  console.groupEnd();
}
function vaciarPorClaveLocalStore(clave, tipo = 'array') {

  const estructura = tipo === 'object' ? {} : [];

  localStorage.setItem(clave, JSON.stringify(estructura));
  if(clave==='kaizenRegistrados')console.log('Kaizen sin datos almacenados')

}
const listaLocalEmpleados = [
  'Diana Catalina Mira Londoño',
  'Juan Carlos Perez Gomez',  
  'Maria Fernanda Lopez Ruiz',
  'Luis Alberto Martinez Castro',
  'Ana Sofia Ramirez Torres',
  'Carlos Andres Hernandez Diaz',
  'Laura Vanessa Morales Rojas',
  'Jorge Ivan Castillo Vargas',
  'Diana Carolina Ortiz Mendoza',
  'Pedro Antonio Navarro Gil',
  'Sandra Milena Cruz Pineda',
  'Miguel Angel Rios Salazar',
  'Paula Andrea Guerrero Silva',
  'Ricardo Javier Vega Campos',
  'Camila Alejandra Nunez Leon',
  'Diego Fernando Cabrera Soto',
  'Natalia Patricia Herrera Luna',
  'Andres Felipe Medina Peña',
  'Valentina Isabel Cortes Parra',
  'Sebastian David Quintero Rivas',
  'Daniela Marcela Escobar Fuentes',
  'n/a'
];
const listaLocalClientes = [
  'Cliente A',
  'Cliente B',
  'Cliente C',
  'Cliente D',
...
if ("IntersectionObserver" in window) {
 ...
 const colorActual = document.querySelector('#current-Color');   
  ...
  irAconsola.addEventListener('click', () =>{
    ...
    let elementosExcluidos = ['simulador','interfaz-perfiles','perfiles-entintado','boton-perfiles','boton-reseteo','spn-blur-1','spn-blur-2','spn-blur-3','spn-blur-4','spn-blur-5','spn-blur-6','spn-blur-7','spn-blur-8']
...
function abrirInterfaz() {
  ...
  const elementosExcluidos = [
    'videoBackgroundII','video-background','padre-interfaz',
...
function showButtonsTeoriaConRetraso() {
  let botones = document.querySelectorAll('.butt-prepre');
...
function showButtonsDensitoConRetraso() {
  let botones = document.querySelectorAll('.deploy-butt');
  for (let i = 0; i < botones.length; i++) {
      botones[i].style.display = 'none';
  }
  function mostrarBotonConRetraso(i) {
...
function cambioContenedor(elementId){
  let elementsToHide = document.querySelectorAll('.alimentador, .uTeñido, .desbobinador,.unidProceso,.rebobinador,.links-inicialesI,.links-iniciales') 
  let torreImpresion = document.getElementById('torre-imp')
  let botonTorre = document.getElementById('toggleVideoButton')
  let botAtras = document.getElementById('bot-atras10')
  let botTrasII = document.getElementById('bot-atras11')
...
function secuenciaEntrena(){
  ...
  let elementosExcluidos = ['padre-1','imag1','instrucciones_manometro','vid_festo','control-neumatico']  
  for (let i = 0; i < allContenedores.length; i++) { 
    let elemento = document.getElementById(allContenedores[i])  
    if (elemento) {
      elemento.style.display = elementosExcluidos.includes(allContenedores[i]) ? 'flex' : 'none'
...
function agregarEmpleado() {
  ...
  // 1. Cargar colección directamente desde localStorage
  ...
  const identificados = document.getElementById('numDoc1-ma').value.trim();
  const corregidos = document.getElementById('numDoc2-ma').value.trim(); 
  const tipoA = document.getElementById('numDoc5-ma').value.trim();
  const tipoB = document.getElementById('numDoc3-ma').value.trim();
  const kaizen = document.getElementById('numDoc4-ma').value.trim();
  const adas = document.getElementById('numDoc7-ma').value.trim();
  const adt = document.getElementById('numDoc6-ma').value.trim();
  const lup = document.getElementById('numDoc8-ma').value.trim();
  ...
  const nuevoEmpleado = new Empleado(
    nombre, documento, area, cargo, equipo, fecha, imagen,
    identificados, corregidos, tipoA, tipoB, kaizen, adas, adt, lup
  );

  empleadoGlobal = nuevoEmpleado;

  // 6. Insertar en la colección local
  empleadosLocal.push(nuevoEmpleado);

  // 7. Guardar definitivamente
  localStorage.setItem(empleadosKey, JSON.stringify(empleadosLocal));

  // 8. Actualizar imagen renderizada
  if (imgEmpleado) {
...
function llenarOpcionesKaizen() {
  ...
  if (!selectImagenKaizen || !contenedorKaizen) return;

  const valorActual = selectImagenKaizen.value;

  selectImagenKaizen.innerHTML = '<option value="">Seleccione...</option>';

  const celdas = contenedorKaizen.querySelectorAll('.cell');

  celdas.forEach(celda => {
    const img = celda.querySelector('img');
    if (!img) return;

    const ruta = img.dataset.src?.trim();
    if (!ruta) return;

    const option = document.createElement('option');
    option.value = ruta;
    option.textContent = ruta;
    selectImagenKaizen.appendChild(option);
  });

  if (valorActual) {
    selectImagenKaizen.value = valorActual;
  }
}
const selectImagenKaizen = document.getElementById('imagen-kaizen');
(() => {
  const selectImagenKaizen = document.getElementById('imagen-kaizen');
  const contenedorKaizen = document.getElementById('kaizen-propuestos');
  const pantallaKaizen = document.getElementById('pantalla-kaizen');

  if (!selectImagenKaizen || !contenedorKaizen || !pantallaKaizen) return;

  const labelPantalla = pantallaKaizen.querySelector('label');
  const imgPantalla = pantallaKaizen.querySelector('img');

  if (!labelPantalla || !imgPantalla) return;

  selectImagenKaizen.addEventListener('click', llenarOpcionesKaizen);

  selectImagenKaizen.addEventListener('change', (e) => {
    const rutaSeleccionada = e.target.value.trim();
    if (!rutaSeleccionada) return;

    const celdaCoincidente = contenedorKaizen.querySelector(`img[data-src="${rutaSeleccionada}"]`)?.closest('.cell');
    const texto = celdaCoincidente?.querySelector('.label')?.textContent?.trim() || rutaSeleccionada;

    imgPantalla.src = rutaSeleccionada;
    imgPantalla.dataset.src = rutaSeleccionada;
...
function agregarKaizen() {
  ...
  const raw = JSON.parse(localStorage.getItem(kaizenKey));
  const kaizenLocal = Array.isArray(raw) ? raw : [];

  const empleado = document.getElementById('empleado').value.trim();
  const propuesta = document.getElementById('mejora').value.trim();
  const fecha = document.getElementById('fecha').value.trim();
  const autogestionado = document.getElementById('autogestionado').value.trim();
  const imagen = document.getElementById('imagen-kaizen').value.trim();
  const participante1 = document.getElementById('participante1').value.trim();
  const participante2 = document.getElementById('participante2').value.trim();
  const participante3 = document.getElementById('participante3').value.trim();
  const participante4 = document.getElementById('participante4').value.trim();
  const participante5 = document.getElementById('participante5').value.trim();
  const lider = document.getElementById('lider').value.trim();
  const equipo = document.getElementById('equipo').value.trim();
  const area = document.getElementById('area').value.trim();

  if (!empleado || !autogestionado || !propuesta || !imagen) {
    saltarAlerta('Hay campos sin diligenciar.', 'fichaTecnica');
    return;
  }
  ...
  const propuestaNormalizada = propuesta.toLowerCase();

  const propuestaDuplicada = kaizenLocal.some(kaizen =>
    (kaizen.propuesta || '').trim().toLowerCase() === propuestaNormalizada
  );

  if (propuestaDuplicada) {
    saltarAlerta('Esta propuesta de Kaizen ya existe.', 'fichaTecnica');
    parpadearElemento('mejora', 150, 2500);
    return;
  }

  kaizenLocal.push(nuevoKaizen);
  localStorage.setItem(kaizenKey, JSON.stringify(kaizenLocal));

  saltarAlerta('Propuesta Kaizen registrada correctamente.', 'fichaTecnica');

  mostrarPropuestas(empleado);
}
// ---------- EVENTO ----------
...
btnModificar.addEventListener('click', () =>{
  if(flagEmpleado === false){
    alternarColor(btnDblFlecha,btnModificar,500)
    return
  }
...
btnLimpiar.addEventListener('click', () =>{
  if(flagEmpleado === false){
    alternarColor(btnDblFlecha,btnLimpiar,500)
    return
  }
  limpiarEntradas()
})
btnSalirRH.addEventListener('click',()=>{
  if(flagEmpleado === false){
    alternarColor(btnDblFlecha,btnSalirRH,500)
    return
  }  
...
const integrantes = Array.from(document.querySelectorAll('.first_table'));
// Duración distinta para cada hijo (subida y bajada)
const DURACIONES = [2800, 3500, 2200, 4000, 3100];
let running = false;
const rafIds = [];
function animarHijo(el, duracion) {
  return new Promise(resolve => {
    const start = performance.now();
    const desde = parseFloat(el.style.height) || 0;
    const hasta = el._subiendo ? 100 : 0;
    ...
    function step(now) {
      ...
      el.style.height = valor + '%';
      if (t < 1) {
        const id = requestAnimationFrame(step);
        rafIds.push(id);
      } else {
        el.style.height = hasta + '%';
        resolve();
      }
    }
    const id = requestAnimationFrame(step);
...
async function bucleHijo(el, duracion) {
  el._subiendo = true;
  while (running) {
    await animarHijo(el, duracion);
    if (!running) break;
    el._subiendo = !el._subiendo;
  }
}
function startAnim() {
  if (running) return;
  running = true;
  integrantes.forEach(h => h.style.height = '0%');

  integrantes.forEach((h, i) => {
    // Offset inicial aleatorio para que no arranquen todos desde 0 al mismo tiempo
    const delay = Math.random() * DURACIONES[i];
    h._subiendo = true;
    setTimeout(() => {
      if (running) bucleHijo(h, DURACIONES[i]);
    }, delay);
  });
}
function stopAnim() {
...
const hijosB = Array.from(document.querySelectorAll('.second_table'));
const DURACIONES_B = [3200, 2600, 3800, 2400, 3500];
let runningB = false;
const rafIdsB = [];
function animarHijoB(el, duracion) {
  return new Promise(resolve => {
    const start = performance.now();
    const desde = parseFloat(el.style.height) || 0;
    const hasta = el._subiendo ? 100 : 0;
    ...
    function step(now) {
      if (!runningB) { resolve(); return; }
      const t = Math.min((now - start) / duracion, 1);
      const eased = 1 - Math.pow(1 - t, 3);
      el.style.height = (desde + (hasta - desde) * eased) + '%';
      if (t < 1) {
        const id = requestAnimationFrame(step);
        rafIdsB.push(id);
      } else {
...
async function bucleHijoB(el, duracion) {
  el._subiendo = true;
  while (runningB) {
    await animarHijoB(el, duracion);
    if (!runningB) break;
    el._subiendo = !el._subiendo;
  }
}
function startAnimB() {
  ...
  hijosB.forEach((h, i) => {
    const delay = Math.random() * DURACIONES_B[i];
    h._subiendo = true;
    setTimeout(() => {
      if (runningB) bucleHijoB(h, DURACIONES_B[i]);
    }, delay);
  });
}
function stopAnimB() {
  runningB = false;
  rafIdsB.forEach(id => cancelAnimationFrame(id));
  rafIdsB.length = 0;
  hijosB.forEach(h => h.style.height = '0%');
}